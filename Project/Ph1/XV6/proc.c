...
get_table_pinfo(void)
{
  struct proc *p;
  acquire(&ptable.lock);
  //initialize pinfotable
  int idx = 0;
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
    if (p->state == RUNNING || p->state == RUNNABLE)
    {
      pinfotable.procInfo[idx].pid = p->pid;
      pinfotable.procInfo[idx].memsize = p->sz;
      idx++;
    }
  release(&ptable.lock);
  bubble_sort(pinfotable.procInfo, idx);
  return pinfotable.procInfo;
}